...
raw = LOAD '/user/chenhuichun/Input/category_entry_avg.csv' USING PigStorage(',') AS  --/user/chenhuichun/Input/
(category: chararray,
	entry: chararray,
	avg_score: double); 
...
{
	projected = FOREACH raw GENERATE entry, avg_score;
...
/*
grunt> run task9.pig;
grunt> fs -getmerge task9/ category_entry_avg_sort.csv;
